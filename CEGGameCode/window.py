...
pg.display.set_icon(pg.image.load(resource_path("img/CEG_Icon.png")))
...
# 메인 배경음악
background_sound = pg.mixer.Sound(resource_path("bgm/bgm.wav"))
background_sound.play(-1)
# 게임화면 배경음악
in_game_music = pg.mixer.Sound(resource_path("bgm/in_game_music.wav"))

#버튼 클릭음
buttonClick = pg.mixer.Sound(resource_path("bgm/button_click.wav"))


clock = pg.time.Clock()
titleImg = pg.image.load(resource_path("img/startimg.png"))
board = pg.image.load(resource_path("img/board.png"))
board = pg.transform.scale(board, (800, 800))

startImg = pg.image.load(resource_path("img/starticon.png"))
clickStartImg = pg.image.load(resource_path("img/clickedStartIcon.png"))

quitImg = pg.image.load(resource_path("img/quiticon.png"))
clickQuitImg = pg.image.load(resource_path("img/clickedQuitIcon.png"))

diceImg = pg.image.load(resource_path("img/dice.png"))
diceImg = pg.transform.scale(diceImg, (150, 150))
sideImg = pg.image.load(resource_path("img/side.png"))

yesImg = pg.image.load(resource_path("img/Yes.png"))
yesImg = pg.transform.scale(yesImg, (470, 200))

clickYesImg = pg.image.load(resource_path("img/clickYes.png"))
clickYesImg = pg.transform.scale(clickYesImg, (470, 200))

noImg = pg.image.load(resource_path("img/No.png"))
noImg = pg.transform.scale(noImg, (470, 200))

clickNoImg = pg.image.load(resource_path("img/clickNo.png"))
clickNoImg = pg.transform.scale(clickNoImg, (470, 200))

winImg = pg.image.load(resource_path("img/win.png"))

gameoverImg = pg.image.load(resource_path("img/gameover.png"))

restartImg = pg.image.load(resource_path("img/restart.png"))
restartImg = pg.transform.scale(restartImg, (180, 180))


clickRestartImg = pg.image.load(resource_path("img/clickedrestart.png"))
...
selectText = pg.image.load(resource_path("img/select.png"))

cr1 = pg.image.load(resource_path("img/c1.png"))
cr1 = pg.transform.scale(cr1, (170, 170))

clickedcr1Img = pg.image.load(resource_path("img/click1.png"))
clickedcr1Img = pg.transform.scale(clickedcr1Img, (170, 170))

cr2 = pg.image.load(resource_path("img/c2.png"))
cr2 = pg.transform.scale(cr2, (170, 170))

clickedcr2Img = pg.image.load(resource_path("img/click2.png"))
...
class DiceButton:
    ...
    def rollDice(self):
        ...
        dice_sound = pg.mixer.Sound(resource_path("bgm/Rolls_Dice.wav"))
        dice_sound.play()
...
def printText2(msg, color, size, pos=(600, 600)):
    font = pg.font.SysFont(resource_path("font/Jalan.ttf"), size)
    textSurface = font.render(msg, True, pg.Color(color))
    textRect = textSurface.get_rect()
    textRect.topleft = pos
    screen.blit(textSurface, textRect)
...
def printKorean(msg, color, size, pos=(600, 600)):
    font = pg.font.Font(resource_path("font/BMHANNAPro.ttf"), size)
    textSurface = font.render(msg, True, pg.Color(color))
    textRect = textSurface.get_rect()
    textRect.topleft = pos
    screen.blit(textSurface, textRect)
...
def printSpace(msg, color, size, pos=(600, 600)):
    font = pg.font.Font(resource_path("font/aAbstractGroovy.ttf"), size)
    textSurface = font.render(msg, True, pg.Color(color))
    textRect = textSurface.get_rect()
    textRect.topleft = pos
    screen.blit(textSurface, textRect)
...
def gameScreen(img_act):
    ...
    #카드뽑기음악
    card_music = pg.mixer.Sound(resource_path("bgm/card.wav"))

    #블랙홀음악
    jail_sound = pg.mixer.Sound(resource_path("bgm/jail.wav"))

    #패배음악
    gameover_sound = pg.mixer.Sound(resource_path("bgm/gameover.wav"))
    win_sound = pg.mixer.Sound(resource_path("bgm/winmusic.wav"))
    ...
    while not gameexit:
        ...
        #폰트
        turn = pg.font.Font(resource_path("font/hangul.ttf"),60)
        korean = pg.font.Font(resource_path("font/hangul.ttf"),40)
        sideNum = pg.font.Font(resource_path("font/hangul.ttf"),50)
        ...
        font7 = sideNum.render('1000', True, BLACK)
        ...
        # 무료 우주여행칸에 도착했을경우    
            ...
            if timerWait == None and timerAssignBlock == False:
                ...
                random_sound = pg.mixer.Sound(resource_path("bgm/random.wav"))
                random_sound.play() 
        ...
        #게임 승리 or 패배 조건        
        if turnCnt == 30 or fuel <= 0 or resource >= 1000:
            if resource < 1000:
                pressSpaceBlock = False
                resourceButtonBlock = True
                pg.draw.rect(screen, BABYPINK, [20, 80, 760, 650], 0, 30)
